...
# ================= MOVEMENT =================
async def move_with_images(call, target_group, buttons=None):
...
async def move_design(call: types.CallbackQuery):
    ...
    await move_with_images(call, GROUP_DESIGN, design_buttons())
...
async def move_ready(call: types.CallbackQuery):
    ...
    call.message.text = text
    await move_with_images(call, GROUP_READY, ready_buttons())
...
async def back_from_issues(call: types.CallbackQuery):
    ...
    if call.data == "back_new":
        await move_with_images(call, GROUP_NEW, new_buttons())
